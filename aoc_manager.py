...
def run_mod(mod, input_text, parts):
    return mod.solve(mod.parse_input(input_text), parts)
...
def run(args):
    ...
    if day := args.day:
        dir_names = filter(lambda dir_name: dir_name.startswith(str(day)), dir_names)

    answers = {}

    for d in dir_names:
        day, problem_title = d.split('_', maxsplit=1)
        answers[day] = run_mod(import_module(d), get_input_text(d), list(map(int, args.part))) # f"Day {day} — {' '.join(problem_title.split('_')).title()}"
...
run_parser.add_argument('--part', '-p', metavar='P', type=list, default=[1, 2],
